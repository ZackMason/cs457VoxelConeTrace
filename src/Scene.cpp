...
void ImageManipulation::run(RenderState& gl_state)
{
    f32 theta = 1.f;
    f32 power = 1.f;
    f32 blend = 0.f;
    ...
    while(!glfwWindowShouldClose(gl_state.window))
    {
        ...
        if(glfwGetKey(gl_state.window, GLFW_KEY_P) == GLFW_PRESS)
        {
            image_manip_shader = Shader("image", {"image.fs", "image.vs"});
        }
        if(glfwGetKey(gl_state.window, GLFW_KEY_Q) == GLFW_PRESS)
        {
            theta += dt;
        }
        ...
        if(glfwGetKey(gl_state.window, GLFW_KEY_E) == GLFW_PRESS)
        {
            blend += dt;
        }
        ...
        if(glfwGetKey(gl_state.window, GLFW_KEY_R) == GLFW_PRESS)
        {
            contrast += dt;
        }
        if(glfwGetKey(gl_state.window, GLFW_KEY_F) == GLFW_PRESS)
        {
            contrast -= dt;
        }
        
        image_manip_shader.bind();
        image1->set_texture_slot(0);
        image1->bind(gl_state);
        image_manip_shader.set_int("uTexture1", 0);
        
        image2->set_texture_slot(1);
        image2->bind(gl_state);
        image_manip_shader.set_int("uTexture2", 1);
        
        
        image_manip_shader.set_float("uTheta", theta);
        image_manip_shader.set_float("uPower", power);
        image_manip_shader.set_float("uBlend", blend);
        image_manip_shader.set_float("uContrast", contrast);
        
        screen.draw(gl_state);
